...
JNIEXPORT jboolean JNICALL Java_io_omniedge_n2n_N2NService_startEdge(
    ...
    jclass cls = (*env)->FindClass(env, "io/omniedge/n2n/model/EdgeStatus");
    if (!cls) {
        ResetEdgeStatus(env, 1 /* cleanup*/);
        return JNI_FALSE;
    }
    status.jcls_status = (*env)->NewGlobalRef(env, cls);
    jclass cls_rs = (*env)->FindClass(env, "io/omniedge/n2n/model/EdgeStatus$RunningStatus");
    if (!cls_rs) {
        ResetEdgeStatus(env, 1 /* cleanup*/);
        return JNI_FALSE;
    }
...
JNIEXPORT void JNICALL Java_io_omniedge_n2n_N2NService_stopEdge(
        JNIEnv *env,
        jobject this) {
...
JNIEXPORT jobject JNICALL Java_io_omniedge_n2n_N2NService_getEdgeStatus(
        JNIEnv *env,
        jobject this) {
    ...
    jobject jRunningStatus = (*env)->GetStaticObjectField(env, cls_rs,
                                                          (*env)->GetStaticFieldID(env, cls_rs,
                                                                                   running_status,
                                                                                   "Lio/omniedge/n2n/model/EdgeStatus$RunningStatus;"));
...
void report_edge_status(void) {
    ...
    mid = (*env)->GetMethodID(env, cls, "reportEdgeStatus",
                              "(Lio/omniedge/n2n/model/EdgeStatus;)V");
    if (!mid) {
        (*env)->DeleteLocalRef(env, jRunningStatus);
        (*env)->DeleteLocalRef(env, jStatus);
        return;
