...
static ssize_t sendto_sock(const n2n_edge_t * eee, int fd, const void * buf,
  ...
  fill_sockaddr((struct sockaddr *) &peer_addr,
    ...
    eee->device.sendto_sock(fd, buf, len, (const char *)ip, (int)ntohs(peer_addr.sin_port));
#else
...
static void send_register_super(n2n_edge_t * eee,
  ...
  /* sent = */ sendto_sock(eee, eee->udp_sock, pktbuf, idx, supernode);
}
...
static void send_query_peer( n2n_edge_t * eee,
  ...
  sendto_sock(eee, eee->udp_sock, pktbuf, idx, &(eee->supernode) );
}
...
static void send_register(n2n_edge_t * eee,
  ...
  if(eee->conf.header_encryption == HEADER_ENCRYPTION_ENABLED)
    packet_header_encrypt (pktbuf, idx, eee->conf.header_encryption_ctx,
			   eee->conf.header_iv_ctx, pearson_hash_16 (pktbuf, idx));
  /* sent = */ sendto_sock(eee, eee->udp_sock, pktbuf, idx, remote_peer);
}
...
static void readFromMgmtSocket(n2n_edge_t * eee, int * keep_running) {
  ...
  i = sizeof(sender_sock);
    ...
    char ip[64];
    int port = 0;
    recvlen = eee->device.recvfrom(eee->udp_mgmt_sock, udp_buf, N2N_PKT_BUF_SIZE, ip, &port);
#else
  recvlen = recvfrom(eee->udp_mgmt_sock, udp_buf, N2N_PKT_BUF_SIZE, 0/*flags*/,
		     (struct sockaddr *)&sender_sock, (socklen_t*)&i);
#endif //#endif //__IOS_PLATFORM
...
static int send_packet(n2n_edge_t * eee,
  ...
  /* s = */ sendto_sock(eee, eee->udp_sock, pktbuf, pktlen, &destination);
  return 0;
}
...
static void readFromIPSocket(n2n_edge_t * eee, int in_sock) {
  ...
  time_t              now=0;
    ...
    char ip[64] = {0};
    int port = 0;
    recvlen = eee->device.recvfrom(in_sock, udp_buf, N2N_PKT_BUF_SIZE, ip, &port);
#else
  size_t              i;
  i = sizeof(sender_sock);
  ...
  sender.family = AF_INET; /* UDP socket was opened PF_INET v4 */
    ...
    memcpy(&(sender.addr.v4), &addr, IPV4_SIZE);
#else
...
#ifdef __IOS_PLATFORM__
    eee->udp_sock = eee->device.upd_socket;
#else
...
#ifdef __IOS_PLATFORM__
  eee->udp_mgmt_sock = eee->device.mgr_socket;
#else
